...
export default function BodyChecksPage() {
  ...
  const columns = [
    ...
    { key: 'aiResultSummary', header: 'Tóm tắt kết quả AI' },
    ...
    { key: 'createdDate', header: 'Ngày tạo' },
    {
      ...
      render: (row) => (
        <div className="flex flex-wrap gap-2">
          ...
          <button className="btn-secondary" onClick={() => review(row)}><CheckCircle2 size={15} /> Đã duyệt</button>
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <h2 className="text-lg font-black text-slate-950">Kiểm tra vóc dáng</h2>
        <p className="text-sm text-slate-500">Theo dõi ảnh quét đã tải lên, tóm tắt AI và trọng tâm tập luyện được đề xuất.</p>
      </div>

      <DataTable columns={columns} data={rows} loading={loading} />

      <Modal open={!!detail} title="Chi tiết kiểm tra vóc dáng" onClose={() => setDetail(null)}>
        {detail ? (
          <div className="grid gap-5 lg:grid-cols-[280px_1fr]">
            ...
            <div className="space-y-4">
              ...
              <p className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm font-semibold leading-6 text-amber-800">
                Dữ liệu quét vóc dáng chỉ nên được xem xét như định hướng tập luyện. Các vấn đề y tế cần được xử lý ngoài quy trình quản trị này.
              </p>
              <div>
                <h4 className="text-sm font-black text-slate-950">Kết quả AI</h4>
                <p className="mt-2 text-sm leading-6 text-slate-600">{detail.aiResultSummary}</p>
              </div>
              <div>
                <h4 className="text-sm font-black text-slate-950">Nhóm cơ được đề xuất</h4>
                <div className="mt-2 flex flex-wrap gap-2">
                  {detail.suggestedMuscleGroups.map((item) => <Badge key={item}>{item}</Badge>)}
                </div>
              </div>
              <div>
                <h4 className="text-sm font-black text-slate-950">Bài tập được đề xuất</h4>
              ...
              <div className="flex justify-end">
                <button className="btn-primary" onClick={() => review(detail)}><CheckCircle2 size={16} /> Đánh dấu đã duyệt</button>
              </div>
            </div>
          </div>
        ) : null}
      ...
      <ConfirmDialog open={!!deleteTarget} title="Xóa lượt kiểm tra vóc dáng" message={`Xóa lượt kiểm tra vóc dáng của ${deleteTarget?.user}?`} onCancel={() => setDeleteTarget(null)} onConfirm={remove} confirmText="Xóa" />
    </div>
  )
}
